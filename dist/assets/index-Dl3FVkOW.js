...
// === ЭФФЕКТ ВОЙДА (засосан Владыкой пустоты) ===
vec3 renderVoid(vec2 uv, float time, vec3 cameraPos) {
  // Тёмно-фиолетовый фон с градиентом
  vec3 voidColor = vec3(0.02, 0.0, 0.05);
  vec3 purpleGlow = vec3(0.15, 0.0, 0.25);
  
  // Градиент от центра
  float centerDist = length(uv - 0.5);
  voidColor = mix(purpleGlow, voidColor, smoothstep(0.0, 0.7, centerDist));
  
  // === ЛЕТЯЩИЕ ЛИНИИ (speed lines) - снизу вверх ===
  float lines = 0.0;
  
  // Множество слоёв линий с разной скоростью
  for (float layer = 0.0; layer < 5.0; layer++) {
    float layerSpeed = 3.0 + layer * 2.0;
    float layerDensity = 30.0 + layer * 20.0;
    float layerBrightness = 0.8 - layer * 0.12;
    
    // Позиция линии
    vec2 lineUV = uv;
    lineUV.y = fract(lineUV.y * 0.5 - time * layerSpeed + layer * 0.1);
    
    // Случайное распределение по X
    float lineX = floor(uv.x * layerDensity);
    float lineRand = fract(sin(lineX * 127.1 + layer * 311.7) * 43758.5453);
    
    // Толщина линии
    float lineWidth = 0.3 + lineRand * 0.4;
    float lineCenter = fract(lineRand * 7.0);
    
    // Расстояние до центра линии
    float lineDist = abs(fract(uv.x * layerDensity) - lineCenter);
    
    // Линия видна только если достаточно близко
    float line = smoothstep(lineWidth * 0.5, 0.0, lineDist);
    
    // Длина линии (вертикальная)
    float lineLength = 0.1 + lineRand * 0.3;
    line *= smoothstep(0.0, 0.05, lineUV.y) * smoothstep(lineLength, lineLength * 0.7, lineUV.y);
    
    // Случайное появление
    line *= step(0.6, lineRand);
    
    lines += line * layerBrightness;
  }
  
  // Цвет линий - от белого до фиолетового
  vec3 lineColor = mix(vec3(0.6, 0.3, 1.0), vec3(1.0, 0.9, 1.0), lines);
  voidColor += lineColor * lines * 0.6;
  
  // === ПУЛЬСИРУЮЩИЕ КОЛЬЦА от центра ===
  float ringDist = length(uv - 0.5);
  for (float i = 0.0; i < 4.0; i++) {
    float ringRadius = fract(time * 0.3 + i * 0.25) * 0.8;
    float ring = smoothstep(0.02, 0.0, abs(ringDist - ringRadius));
    ring *= 1.0 - ringRadius; // Затухание к краю
    voidColor += vec3(0.4, 0.1, 0.8) * ring * 0.5;
  }
  
  // === ТЁМНЫЕ ЧАСТИЦЫ (плавающие сгустки тьмы) ===
  for (float i = 0.0; i < 8.0; i++) {
    float px = fract(sin(i * 73.7) * 43758.5453);
    float py = fract(cos(i * 127.1) * 43758.5453);
    float pSpeed = 0.5 + fract(sin(i * 37.3) * 1000.0) * 1.0;
    
    vec2 particlePos = vec2(px, fract(py - time * pSpeed));
    float particleDist = length(uv - particlePos);
    float particleSize = 0.02 + fract(sin(i * 91.7) * 100.0) * 0.03;
    
    float particle = smoothstep(particleSize, 0.0, particleDist);
    voidColor -= vec3(0.03, 0.0, 0.02) * particle; // Тёмные частицы
  }
  
  // === ИСКАЖЕНИЕ ПО КРАЯМ (виньетка) ===
  float vignette = 1.0 - smoothstep(0.3, 0.8, centerDist);
  voidColor *= 0.5 + vignette * 0.5;
  
  // === МЕРЦАНИЕ ===
  float flicker = 0.9 + 0.1 * sin(time * 15.0);
  voidColor *= flicker;
  
  // Фиолетовое свечение по краям экрана
  float edgeGlow = smoothstep(0.3, 0.6, centerDist);
  voidColor += vec3(0.2, 0.0, 0.4) * edgeGlow * 0.3;
...
void main() {
  ...
  // === РЕЖИМ ВОЙДА - специальный рендеринг ===
  if (u_voidMode == 1) {
    // Фон войда с летящими линиями
    vec3 voidBg = renderVoid(v_uv, u_time, u_cameraPos);
    
    // Рендерим только врагов (фантомов) в войде
    float d = MAX_DIST;
    int hitMat = 0;
    vec3 hitPos = ro;
    vec3 hitNormal = vec3(0.0, 1.0, 0.0);
    
    // Ray march только для врагов
    float marchDist = 0.0;
    for (int i = 0; i < MAX_STEPS; i++) {
      vec3 p = ro + rd * marchDist;
      float minD = MAX_DIST;
      
      // Только враги в войде
      for (int j = 0; j < u_targetCount; j++) {
        if (j >= 16) break;
        vec4 target = u_targets[j];
        if (target.w > 0.5) {
          vec3 tp = p - target.xyz;
          float enemyD = length(tp) - 0.6; // Фантом
          if (enemyD < minD) {
            minD = enemyD;
            hitMat = 5; // phantom
          }
    ...
    // Если попали во врага - рендерим его
    if (d < MAX_DIST) {
      // Фантом в войде - более яркий и заметный
      vec3 enemyColor = vec3(0.1, 0.0, 0.15);
      vec3 edgeGlow = vec3(0.5, 0.0, 0.8);
      
      // Вычисляем нормаль
      vec2 e = vec2(0.02, 0.0);
      vec3 n = normalize(vec3(1.0)); // Упрощённая нормаль для сферы
      ...
      color = mix(enemyColor, edgeGlow, fresnel);
      color *= 2.5;
      
      // Искры
      float spark = sin(u_time * 20.0 + hitPos.y * 10.0);
      if (spark > 0.9) {
        color += vec3(0.8, 0.3, 1.0);
      }
    }
...
`;class N{constructor(t){this.canvas=t;const e=t.getContext("webgl2");if(!e)throw new Error("WebGL2 не поддерживается");this.gl=e,this.uniforms={resolution:null,time:null,cameraPos:null,cameraDir:null,cameraYaw:null,cameraPitch:null,targets:null,targetCount:null,muzzleFlash:null,pools:null,poolCount:null,acidProjectiles:null,acidProjectileCount:null,acidRainZones:null,acidRainZoneCount:null,era:null,wave:null,greenBossPhase2:null,pickups:null,pickupCount:null,crystals:null,crystalCount:null,voidMode:null,voidProgress:null},this.init()}gl;program=null;uniforms;vao=null;renderScale=.75;init(){const t=this.gl,e=this.compileShader(t.VERTEX_SHADER,X),i=this.compileShader(t.FRAGMENT_SHADER,j);if(!e||!i)throw new Error("Ошибка компиляции шейдеров");if(this.program=t.createProgram(),t.attachShader(this.program,e),t.attachShader(this.program,i),t.linkProgram(this.program),!t.getProgramParameter(this.program,t.LINK_STATUS))throw new Error("Ошибка линковки: "+t.getProgramInfoLog(this.program));this.uniforms.resolution=t.getUniformLocation(this.program,"u_resolution"),this.uniforms.time=t.getUniformLocation(this.program,"u_time"),this.uniforms.cameraPos=t.getUniformLocation(this.program,"u_cameraPos"),this.uniforms.cameraYaw=t.getUniformLocation(this.program,"u_cameraYaw"),this.uniforms.cameraPitch=t.getUniformLocation(this.program,"u_cameraPitch"),this.uniforms.targets=t.getUniformLocation(this.program,"u_targets"),this.uniforms.targetCount=t.getUniformLocation(this.program,"u_targetCount"),this.uniforms.muzzleFlash=t.getUniformLocation(this.program,"u_muzzleFlash"),this.uniforms.pools=t.getUniformLocation(this.program,"u_pools"),this.uniforms.poolCount=t.getUniformLocation(this.program,"u_poolCount"),this.uniforms.acidProjectiles=t.getUniformLocation(this.program,"u_acidProjectiles"),this.uniforms.acidProjectileCount=t.getUniformLocation(this.program,"u_acidProjectileCount"),this.uniforms.acidRainZones=t.getUniformLocation(this.program,"u_acidRainZones"),this.uniforms.acidRainZoneCount=t.getUniformLocation(this.program,"u_acidRainZoneCount"),this.uniforms.era=t.getUniformLocation(this.program,"u_era"),this.uniforms.wave=t.getUniformLocation(this.program,"u_wave"),this.uniforms.greenBossPhase2=t.getUniformLocation(this.program,"u_greenBossPhase2"),this.uniforms.pickups=t.getUniformLocation(this.program,"u_pickups"),this.uniforms.pickupCount=t.getUniformLocation(this.program,"u_pickupCount"),this.uniforms.crystals=t.getUniformLocation(this.program,"u_crystals"),this.uniforms.voidMode=t.getUniformLocation(this.program,"u_voidMode"),this.uniforms.voidProgress=t.getUniformLocation(this.program,"u_voidProgress"),this.createQuad()}compileShader(t,e){const i=this.gl,s=i.createShader(t);return s?(i.shaderSource(s,e),i.compileShader(s),i.getShaderParameter(s,i.COMPILE_STATUS)?s:(console.error("Ошибка шейдера:",i.getShaderInfoLog(s)),i.deleteShader(s),null)):null}createQuad(){const t=this.gl,e=new Float32Array([-1,-1,1,-1,-1,1,1,1]);this.vao=t.createVertexArray(),t.bindVertexArray(this.vao);const i=t.createBuffer();t.bindBuffer(t.ARRAY_BUFFER,i),t.bufferData(t.ARRAY_BUFFER,e,t.STATIC_DRAW);const s=t.getAttribLocation(this.program,"a_position");t.enableVertexAttribArray(s),t.vertexAttribPointer(s,2,t.FLOAT,!1,0,0),t.bindVertexArray(null)}resize(){const t=window.devicePixelRatio||1,e=Math.floor(this.canvas.clientWidth*t*this.renderScale),i=Math.floor(this.canvas.clientHeight*t*this.renderScale);(this.canvas.width!==e||this.canvas.height!==i)&&(this.canvas.width=e,this.canvas.height=i,this.gl.viewport(0,0,e,i))}render(t,e,i,s,o,a,n,r,c,h,l,u,p,m,f,d,y,x,b,w,M){const T=this.gl;this.resize(),T.useProgram(this.program),T.bindVertexArray(this.vao),T.uniform2f(this.uniforms.resolution,this.canvas.width,this.canvas.height),T.uniform1f(this.uniforms.time,t),T.uniform3f(this.uniforms.cameraPos,e.x,e.y,e.z),T.uniform1f(this.uniforms.cameraYaw,i),T.uniform1f(this.uniforms.cameraPitch,s),T.uniform4fv(this.uniforms.targets,o),T.uniform1i(this.uniforms.targetCount,a),T.uniform1f(this.uniforms.muzzleFlash,n),r&&c!==void 0?(T.uniform4fv(this.uniforms.pools,r),T.uniform1i(this.uniforms.poolCount,c)):T.uniform1i(this.uniforms.poolCount,0),f&&d!==void 0&&d>0?(T.uniform4fv(this.uniforms.acidProjectiles,f),T.uniform1i(this.uniforms.acidProjectileCount,d)):T.uniform1i(this.uniforms.acidProjectileCount,0),y&&x!==void 0&&x>0?(T.uniform4fv(this.uniforms.acidRainZones,y),T.uniform1i(this.uniforms.acidRainZoneCount,x)):T.uniform1i(this.uniforms.acidRainZoneCount,0),T.uniform1i(this.uniforms.era,h||1),T.uniform1i(this.uniforms.wave,l||1),T.uniform1i(this.uniforms.greenBossPhase2,b?1:0),u&&p!==void 0?(T.uniform4fv(this.uniforms.pickups,u),T.uniform1i(this.uniforms.pickupCount,p)):T.uniform1i(this.uniforms.pickupCount,0),m&&T.uniform4fv(this.uniforms.crystals,m),T.uniform1i(this.uniforms.voidMode,w?1:0),T.uniform1f(this.uniforms.voidProgress,M||0),T.drawArrays(T.TRIANGLE_STRIP,0,4)}destroy(){this.program&&this.gl.deleteProgram(this.program)}}class U{healthEl;ammoEl;fragsEl;crosshairEl;hitmarkerEl;reloadEl;waveEl;messageEl;damageOverlay;weaponEl;constructor(){this.healthEl=document.getElementById("health-value"),this.ammoEl=document.getElementById("ammo-value"),this.fragsEl=document.getElementById("frags-value"),this.crosshairEl=document.getElementById("crosshair"),this.hitmarkerEl=document.getElementById("hitmarker"),this.reloadEl=document.getElementById("reload-indicator"),this.reloadEl&&(this.reloadEl.style.display="none"),this.waveEl=this.createWaveElement(),this.messageEl=this.createMessageElement(),this.damageOverlay=this.createDamageOverlay(),this.weaponEl=this.createWeaponElement()}createWaveElement(){let t=document.getElementById("wave-indicator");return t||(t=document.createElement("div"),t.id="wave-indicator",t.style.cssText=`
    ...
    `,i.textContent=t,document.body.appendChild(i),!document.getElementById("message-anim-style")){const s=document.createElement("style");s.id="message-anim-style",s.textContent=`
      ...
      `,document.head.appendChild(s)}setTimeout(()=>i.remove(),1e3)}rageOverlay=null;bossHealthBar=null;bossHealthFill=null;bossNameEl=null;voidOverlay=null;voidTimerEl=null;voidKillsEl=null;showRageOverlay(t){this.rageOverlay||(this.rageOverlay=document.createElement("div"),this.rageOverlay.id="rage-overlay",this.rageOverlay.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
    ...
    `;const e=document.createElement("div");if(e.style.cssText=`
      ...
      margin-top: 30px;
      letter-spacing: 2px;
    `,e.textContent="РУБИ ТЕНЕВЫХ ОХОТНИКОВ!",!document.getElementById("void-anim-style")){const i=document.createElement("style");i.id="void-anim-style",i.textContent=`
        @keyframes voidPulse {
          0%, 100% { 
            opacity: 1; 
            transform: scale(1);
